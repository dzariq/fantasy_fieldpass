...
interface LeagueInfoClientProps {
    competitionId: string;
    code: string;
}

export default function LeagueInfoClient({
    code,
    competitionId
}: LeagueInfoClientProps) {
    ...
    useEffect(() => {
        async function fetchLeague() {
            ...
            try {
                const response = await fetch('/api/leagues/standings', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({ code, competitionId, phone }),
                });
                ...
                if (response.ok) {
                    ...
                    setIsPaid(leagueData.league_type === 'paid');
                    setEntryFee(leagueData.fee || 0);
                    setTotalCollection(leagueData.total_collection || 0);
                    setTotalPrize(leagueData.total_prize || 0);
                        console.log(leagueData)
                    
                    // Parse prizes from winner_1, winner_2, etc.
                    if (leagueData.league_type === 'paid') {
                        const prizeList: Prize[] = [];
                        
                        // Extract prizes from winner_1 to winner_5
                        for (let i = 1; i <= 5; i++) {
                            const amount = leagueData[`winner_${i}`];
                            if (amount && amount > 0) {
                                prizeList.push({ position: i, amount });
                            }
                        }
                    ...
                    // Find user's rank
                    const userTeam = leagueData.teams?.find((t: Team) => t.entry_id === phone);
                    if (userTeam) {
                        setMyRank(userTeam.rank);
                    }
                }
            } catch (error) {
    ...
    return (
        <div className="standings-page">
                    ...
                    <div className="league-meta">
                        ...
                        {isPaid && (
                            <div className="entry-fee-badge">
                                ...
                                <span>RM {entryFee.toFixed(2)}</span>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {/* Prize Pool Section - Only for Paid Leagues */}
            {isPaid && prizes.length > 0 && (
                <div className="prize-pool-section">
                    <div className="prize-header">
                        <div className="prize-header-left">
                            <h2 className="prize-title">💰 Prize Pool</h2>
                            <div className="pool-breakdown">
                                <span className="breakdown-item">
                                    Collection: <span className="breakdown-value">RM {totalCollection.toFixed(2)}</span>
                                </span>
                            </div>
                        </div>
                        <div className="total-prize">
                            <span className="total-label">Total Prizes</span>
                            <span className="total-amount">
                                RM {totalPrize.toFixed(2)}
                            </span>
                        </div>
                    </div>
                    <div className="prizes-grid">
                        {prizes.map((prize) => (
                            <div key={prize.position} className="prize-card">
                                <div className="prize-position">
                                    <span className="position-icon">{getRankIcon(prize.position)}</span>
                                    <span className="position-text">
                                        {prize.position}{getPositionSuffix(prize.position)} Place
                                    </span>
                                </div>
                                <div className="prize-amount">
                                    RM {prize.amount.toFixed(2)}
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                >
                    <div className="rank-badge">
                        ...
                        <div className="rank-info">
                            ...
                            {isPaid && getPrizeForRank(myRank) > 0 && (
                                <span className="prize-indicator">
                                    💰 RM {getPrizeForRank(myRank).toFixed(2)}
                                </span>
                            )}
                        </div>
                    </div>
                    <div className="rank-stats">
                        ...
                        <div className="stat">
                            <span className="stat-label">Matchweeks</span>
                            <span className="stat-value">{endMw - startMw + 1}</span>
                        </div>
                    </div>
                    ...
                    <div className="podium">
                        ...
                        >
                            ...
                            <div className="team-info">
                                ...
                                {isPaid && getPrizeForRank(2) > 0 && (
                                    <div className="prize-tag">
                                        RM {getPrizeForRank(2).toFixed(2)}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div className="team-info">
                                ...
                                {isPaid && getPrizeForRank(1) > 0 && (
                                    <div className="prize-tag winner">
                                        RM {getPrizeForRank(1).toFixed(2)}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div className="team-info">
                                ...
                                {isPaid && getPrizeForRank(3) > 0 && (
                                    <div className="prize-tag">
                                        RM {getPrizeForRank(3).toFixed(2)}
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <div className="rankings-section">
                ...
                <div className="rankings-table">
                    {teams.map((team) => {
                        const prize = getPrizeForRank(team.rank);
                        return (
                            <div
                                key={team.entry_id}
                                className={`rank-row ${isMyTeam(team.entry_id) ? 'my-team' : ''} ${prize > 0 ? 'winner' : ''} clickable`}
